...
private object UserSQL {
  def insert(user: User): Update0 = sql"""
    INSERT INTO USERS (USER_NAME, EMAIL, HASH, IS_ADMIN)
    VALUES (${user.userName}, ${user.email}, ${user.hash}, ${user.isAdmin})
  """.update

  def update(user: User, id: Long): Update0 = sql"""
    UPDATE USERS
    SET EMAIL = ${user.email}, HASH = ${user.hash}, IS_ADMIN = ${user.isAdmin}
    WHERE ID = $id
  """.update

  def select(userId: Long): Query0[User] = sql"""
    SELECT USER_NAME, EMAIL, HASH, IS_ADMIN, ID
    FROM USERS
    WHERE ID = $userId
  """.query
